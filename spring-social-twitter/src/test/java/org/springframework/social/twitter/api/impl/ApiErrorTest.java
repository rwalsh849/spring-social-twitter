...
public class ApiErrorTest extends AbstractTwitterApiTest {
    ...
    public void tooManyRequests() {
        mockServer
                ...
                .andExpect(method(GET))
                .andRespond(
                        withStatus(HttpStatus.valueOf(429)).body("{\"errors\":[ {\"code\":88, \"message\":\"Rate limit exceeded\"} ] }").contentType(
                                APPLICATION_JSON));
        twitter.searchOperations().search("#spring");
    }
    ...
    public void hourlyRateLimitExceeded() {
        mockServer
                ...
                .andExpect(method(GET))
                .andRespond(
                        withBadRequest().body("{\"error\":\"Rate limit exceeded. Clients may not make more than 350 requests per hour.\"}")
                                .contentType(APPLICATION_JSON));
        twitter.userOperations().getUserProfile();
    }
