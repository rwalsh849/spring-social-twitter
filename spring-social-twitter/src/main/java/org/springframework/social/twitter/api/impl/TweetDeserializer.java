...
public class TweetDeserializer extends JsonDeserializer<Tweet> {
    ...
    public Tweet deserialize(JsonNode node) throws IOException, JsonProcessingException {
        ...
        Tweet tweet = readTweetFromJson(node, id, text);
    ...
    private void readUserDataFromUserNode(JsonNode node, JsonNode fromUserNode, String text, Tweet tweet) throws IOException {
    ...
    private void readReplyParameters(JsonNode node, Tweet tweet) {
        ...
        JsonNode inReplyToScreenNameNode = node.get("in_reply_to_screen_name");
        String inReplyToScreenName = inReplyToScreenNameNode != null && !inReplyToScreenNameNode.isNull() ? inReplyToScreenNameNode.asText() : null;
